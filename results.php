...
$mf = "";
if ($verifiedFilter)           $mf .= " AND verified = 1";
if (!empty($categoryFilter))   $mf .= " AND category = '$categoryFilter'";
if ($ratingFilter > 0)         $mf .= " AND rating >= $ratingFilter";
...
try {
    if ($cityFilter > 0) {
        $r = meiliSearch($cleanQuery, [
            'filter' => "$mf AND city_id = $cityFilter",
        ...
        if ($userCityId > 0) {
            $r1 = meiliSearch($cleanQuery, [
                'filter' => "$mf AND city_id = $userCityId AND country_code = '$countryCode'",
        ...
        $r2 = meiliSearch($cleanQuery, [
            'filter' => "$mf AND country_code = '$countryCode'$cityEx",
            'limit'  => $perPage, 'offset' => $offset,
            'sort'   => ['verified:desc','rating:desc','views:desc'],
        ]);
        if (isset($r2['hits'])) {
        ...
        if ($meiliOk && !empty($cleanQuery) && count($meiliIds) < 5) {
            $r3 = meiliSearch($cleanQuery, [
                'filter' => "$mf AND country_code != '$countryCode'",
...
.mi-blue { background: #EEF2FF; }
.mi-blue svg { stroke: var(--primary); }
.mi-green { background: #F0FDF4; }
.mi-green svg { stroke: #16A34A; }
.mi-orange { background: #FFF7ED; }
.mi-orange svg { stroke: #EA580C; }
.mi-red { background: #FFF1F2; }
.mi-red svg { stroke: var(--danger); }
.mi-slate { background: var(--bg-secondary); }
.mi-slate svg { stroke: var(--text-secondary); }
