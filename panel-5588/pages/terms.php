...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $pdo->prepare("UPDATE pages SET content_html=?, updated_at=NOW() WHERE slug='terms'")
        ->execute([$html]);
    $msg = 'success';
}

$row = $pdo->query("SELECT * FROM pages WHERE slug='terms'")->fetch(PDO::FETCH_ASSOC);

ob_start();
?>
<div class="panel">
    <div class="panel-header">
        <div class="panel-title">📄 Условия использования</div>
        <a href="https://poisq.com/terms.php" target="_blank" class="btn btn-secondary btn-sm">
    ...
    <div style="padding:20px">
        ...
        <p style="font-size:13px;color:var(--text-secondary);margin-bottom:12px">
            Редактируйте HTML-контент страницы. Если поле пусто — показывается статичный контент из <code>terms.php</code>.
        </p>
        <form method="POST">
            <div style="margin-bottom:12px">
                ...
                <textarea name="content_html" class="form-control"
                    ...
                    placeholder="Оставьте пустым чтобы показывать статичный контент из terms.php"><?php echo htmlspecialchars($row['content_html'] ?? ''); ?></textarea>
            </div>
...
renderLayout('Условия использования', $content);
