...
requireModeratorAuth('moderation');
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    header("Location: /mod/moderate.php");
    exit;
}
...
$services = $pdo->query("
    SELECT s.*, s.call_status, s.call_note,
        ...
        <div>
            ...
            <div style="border:1px solid var(--border);border-radius:var(--radius-sm);padding:12px;margin-bottom:14px;<?php echo $callBg; ?>" id="callBlock-<?php echo $svc['id']; ?>">
                ...
                <div id="callNote-<?php echo $svc['id']; ?>" style="<?php echo $callStatus==='other'?'':'display:none;'; ?>margin-bottom:8px;">
                    <textarea id="callNoteText-<?php echo $svc['id']; ?>" class="form-control" rows="2" placeholder="Заметка..."><?php echo htmlspecialchars($callNote); ?></textarea>
                </div>
...
async function approveCity(cityId, svcId) {
    ...
    fd.append('id', cityId);
    const res = await fetch('/mod/cities.php', {method:'POST', body:fd});
    const data = await res.json();
    if (data.ok) {
...
renderModLayout('Модерация', $content);
?>
