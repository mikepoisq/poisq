...
requireModeratorAuth('duplicates');
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($newId > 0) {
        if ($act === "keep_new") {
            ...
            recordModeratorStat(getModeratorId(), $newId, 'approved');
        } elseif ($act === "keep_old") {
    ...
    header("Location: /mod/duplicates.php");
    exit;
}
...
.col-desc { font-size:13px; color:var(--text-secondary); line-height:1.5; max-height:72px; overflow:hidden; margin-bottom:8px; }
...
.user-tag { font-size:11px; color:var(--text-secondary); padding:6px 10px; background:var(--primary-light); border-radius:7px; margin-top:10px; }
.dup-actions { padding:12px 16px; border-top:1px solid var(--border); display:flex; gap:8px; flex-wrap:wrap; background:var(--bg); }
.btn-keep-new { flex:1; min-width:160px; padding:11px 14px; background:var(--success); color:#fff; border:none; border-radius:var(--radius-sm); font-size:13px; font-weight:700; cursor:pointer; font-family:inherit; transition:background 0.15s; }
.btn-keep-new:hover { background:#059669; }
.btn-keep-old { flex:1; min-width:160px; padding:11px 14px; background:var(--bg-white); color:var(--text); border:1.5px solid var(--border); border-radius:var(--radius-sm); font-size:13px; font-weight:600; cursor:pointer; font-family:inherit; }
.btn-keep-old:hover { background:var(--bg); }
.btn-not-dup { flex:1; min-width:160px; padding:11px 14px; background:var(--primary-light); color:var(--primary); border:1.5px solid #BFDBFE; border-radius:var(--radius-sm); font-size:13px; font-weight:600; cursor:pointer; font-family:inherit; }
...
?>
<div class="dup-card">
    <div class="dup-warn">⚠️ Возможный дубль — совпадение по телефону, email или названию</div>
    <div class="dup-cols">
        <!-- НОВЫЙ -->
        <div class="dup-col">
            <span class="col-badge badge-new">● НОВЫЙ</span>
            <?php if (!empty($newPhotos)): ?>
            <div class="col-photos"><?php foreach (array_slice($newPhotos,0,3) as $p): ?><img src="<?php echo htmlspecialchars($p); ?>" alt="" onerror="this.style.display='none'"><?php endforeach; ?></div>
            <?php endif; ?>
            <div class="col-name"><?php echo htmlspecialchars($d["new_name"]); ?></div>
            <div class="col-meta"><?php echo $cat; ?> · <?php echo htmlspecialchars($d["new_city"] ?? "—"); ?><br>Добавлен: <?php echo date("d.m.Y H:i", strtotime($d["new_created"])); ?></div>
            <div class="col-desc" id="nd-<?php echo $d["new_id"]; ?>"><?php echo nl2br(htmlspecialchars($d["new_description"] ?? "")); ?></div>
        ...
        <!-- СУЩЕСТВУЮЩИЙ -->
        <div class="dup-col">
            <span class="col-badge badge-old"><span class="status-dot <?php echo $oldStatusClass; ?>"></span>СУЩЕСТВУЮЩИЙ (<?php echo $oldStatusText; ?>)</span>
            <?php if ($d["old_id"]): ?>
            <?php if (!empty($oldPhotos)): ?>
            <div class="col-photos"><?php foreach (array_slice($oldPhotos,0,3) as $p): ?><img src="<?php echo htmlspecialchars($p); ?>" alt="" onerror="this.style.display='none'"><?php endforeach; ?></div>
            <?php endif; ?>
            <div class="col-name"><?php echo htmlspecialchars($d["old_name"] ?? ""); ?></div>
            <div class="col-meta"><?php echo ($categories[$d["old_category"]] ?? $d["old_category"]); ?> · <?php echo htmlspecialchars($d["old_city"] ?? "—"); ?><br>Добавлен: <?php echo $d["old_created"] ? date("d.m.Y H:i", strtotime($d["old_created"])) : "—"; ?> · <a href="<?php echo serviceUrl($d["old_id"], $d["old_name"]); ?>" target="_blank" style="color:var(--primary)">Открыть ↗</a></div>
            <div class="col-desc" id="od-<?php echo $d["new_id"]; ?>"><?php echo nl2br(htmlspecialchars($d["old_description"] ?? "")); ?></div>
...
$dupCount = $duplicatesCount;
renderModLayout('Дубликаты', $content);
?>
